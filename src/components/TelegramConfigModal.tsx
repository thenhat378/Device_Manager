...
import { Send, Bot, CheckCircle2, AlertCircle, RefreshCw, X, ExternalLink, MessageSquare, Check, Radio, Globe, Zap, Code, Copy, Play } from 'lucide-react';
...
export const TelegramConfigModal: React.FC<TelegramConfigModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'config' | 'code_sample'>('config');
  ...
  // Custom Telegram message content & code snippet sample
  const [noiDungBaoHong, setNoiDungBaoHong] = useState<string>(
    '⚠️ BÁO HỎNG THIẾT BỊ\n- Vị trí: Phòng họp số 3\n- Tình trạng: Máy chiếu không lên nguồn.'
  );
  ...
  // Custom Direct Send Notification Function as requested
  const guiThongBaoTelegram = (tinNhan: string) => {
    const targetToken = botToken.trim() || DEFAULT_BOT_TOKEN;
    const targetChatId = chatId.trim();

    if (!targetChatId) {
      setStatusMessage({
        type: 'error',
        text: 'Vui lòng điền hoặc quét Chat ID trước khi gửi báo hỏng!'
      });
      return;
    }
    ...
    // Gửi yêu cầu đến máy chủ Telegram
    fetch(url, {
      method: 'POST',
      headers: {
    ...
    .then(data => {
      if (data.ok) {
        console.log('Đã gửi báo hỏng thành công đến Telegram!');
        setStatusMessage({
          type: 'success',
          text: `🎉 Đã gửi báo hỏng thành công đến Telegram (@${BOT_USERNAME})!`
        });
      } else {
        console.error('Không thể gửi tin nhắn. Lỗi:', data.description);
        setStatusMessage({
          type: 'error',
          text: `Không thể gửi tin nhắn. Lỗi: ${data.description || 'Lỗi API'}`
        });
      }
    })
    .catch(error => {
      ...
      setStatusMessage({
        type: 'error',
        text: `Lỗi kết nối mạng: ${error.message}`
      });
    })
    .finally(() => {
      setTesting(false);
  ...
  const codeSnippet = `// 1. Cấu hình thông tin bot
const botToken = '${botToken.trim() || 'ĐIỀN_TOKEN_CỦA_BẠN_VÀO_ĐÂY'}'; 
const chatId = '${chatId.trim() || 'ĐIỀN_CHAT_ID_VÀO_ĐÂY'}'; 

// 2. Nội dung thông báo hỏng hóc
const noiDungBaoHong = \`${noiDungBaoHong}\`;

// 3. Hàm gửi tin nhắn
function guiThongBaoTelegram(tinNhan) {
...
// 4. Chạy lệnh gửi báo hỏng
guiThongBaoTelegram(noiDungBaoHong);`;
      ...
      <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="flex border-b border-slate-200 bg-slate-50 px-6 pt-2 gap-2 text-xs font-bold">
          ...
          >
            ...
            Cấu Hình & Quét Chat ID
          </button>

          <button
            type="button"
            onClick={() => setActiveTab('code_sample')}
            className={`pb-2.5 px-3 border-b-2 flex items-center gap-1.5 transition ${
        ...
        <div className="p-6 overflow-y-auto space-y-5 text-xs text-slate-600">
          {activeTab === 'config' ? (
            ...
            <div className="space-y-4">
              {/* Code Sample & Direct Execution Tab */}
              <div className="bg-slate-900 text-slate-200 p-4 rounded-xl border border-slate-800 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Code className="w-4 h-4 text-sky-400" />
                    <span className="font-bold text-white text-xs">Mã Gửi Báo Hỏng Chuẩn Telegram API</span>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="flex items-center gap-1 px-2.5 py-1 rounded-md bg-slate-800 hover:bg-slate-700 text-sky-300 text-[11px] font-medium border border-slate-700 transition"
                  >
                    {copiedCode ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                    {copiedCode ? 'Đã sao chép' : 'Sao chép Code'}
                  </button>
                </div>

                <div className="bg-slate-950 p-3 rounded-lg border border-slate-800 text-[11px] font-mono text-emerald-300/90 overflow-x-auto max-h-56 leading-relaxed">
                  <pre>{codeSnippet}</pre>
                </div>
              </div>
              ...
              <div className="bg-sky-50/70 p-4 rounded-xl border border-sky-200 space-y-3">
                <div className="flex items-center justify-between">
                  <label className="font-bold text-slate-800 text-xs">Nội dung thông báo hỏng hóc cần gửi:</label>
                  <button
                    type="button"
                    onClick={() => setNoiDungBaoHong('⚠️ BÁO HỎNG THIẾT BỊ\n- Vị trí: Phòng họp số 3\n- Tình trạng: Máy chiếu không lên nguồn.')}
                ...
                <textarea
                  rows={3}
                  value={noiDungBaoHong}
                  onChange={(e) => setNoiDungBaoHong(e.target.value)}
                  placeholder="Nhập nội dung thông báo..."
                  className="w-full px-3 py-2 text-xs rounded-xl border border-sky-300 font-medium focus:outline-none focus:border-sky-500 bg-white"
                />

                <div className="flex items-center justify-between pt-1">
                  <span className="text-[11px] text-slate-500">
                    Sẽ gửi đến Chat ID: <strong className="font-mono text-slate-700">{chatId || '(Chưa cấu hình)'}</strong>
                  </span>
                  <button
                    type="button"
                    onClick={() => guiThongBaoTelegram(noiDungBaoHong)}
                    disabled={testing || !chatId.trim()}
                    className="px-4 py-2 rounded-xl bg-gradient-to-r from-sky-600 to-blue-600 hover:from-sky-500 hover:to-blue-500 text-white font-bold text-xs shadow-md transition active:scale-95 flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <Play className="w-3.5 h-3.5 fill-current" />
                    {testing ? 'Đang gửi tin...' : 'Chạy Lệnh Gửi Báo Hỏng'}
                  </button>
                </div>
              </div>
            </div>
        ...
        <div className="px-6 py-3.5 bg-slate-50 border-t border-slate-200 flex items-center justify-between">
          ...
          >
            ...
            {testing ? 'Đang gửi thử...' : '🚀 Bắn Tin Tương Tác 2 Chiều'}
          </button>
